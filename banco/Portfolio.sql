...
DELIMITER //
CREATE PROCEDURE AtualizarPortfolio(IN nome VARCHAR(100), IN email VARCHAR(100), IN dataNascimento VARCHAR(15), IN github VARCHAR(70), IN linkedin VARCHAR(70), IN descricao TEXT, fotoPerfil VARCHAR(100), IN id INT)
BEGIN
	UPDATE Portfolios set nome = nome, email = email, dataNascimento = FormatarDataINSERT(dataNascimento), github = github, linkedin = linkedin, descricao = descricao, fotoPerfil= fotoPerfil WHERE id = 1;
END //
DELIMITER ;
...
BEGIN
	SELECT id, nome, email, FormatarDataSelect(dataNascimento), linkedin, github, descricao FROM Portfolios;
END //
DELIMITER ;
